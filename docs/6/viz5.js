...
function viz5(cb)	{
	...
	d3.request('/_es/121/parking_dbklpgis/_search')
		...
		.post(JSON.stringify(dsl), function(err, raw){
			...
			viz5_render(fEnd, key, '#container6');
		});
...
//
//==================================================================
function viz5_render(cb, key, elid)	{
	var f = arguments.callee.toString().replace(/function\s+/,'').split('(')[0],
			dbg=1, fEnd=function(){ dbg&&console.timeEnd(f); console.groupEnd(f); if (typeof cb=='function') cb() };
	if (dbg){ console.group(f); console.time(f) };
	...
	nest.forEach(function(d){
		...
		scales[d.key] = d3.scaleLinear().domain([d.min,d.max]).range(['crimson','cyan']);
...
//	var scales=d3.scaleLinear().domain([min,max]).range(['black','cyan']);
	...
	sel.append('h3')
		...
		.html('Collective Available Parking Spots by YEAR');
											...
											.call(function(sel)	{
												...
												sel.append('rect')
													.attrs({
														width:w,
														height:h,
														fill:function(d){ return scales[d.values[0][key]](d.values[0].avg) }
													});

												sel.append('text')
													.attrs({
														fill:function(d){ return chroma(scales[d.values[0][key]](d.values[0].avg)).luminance() > .4 ? '#000' : '#fff' },
