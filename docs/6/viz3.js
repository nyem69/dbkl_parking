		...
		.post(JSON.stringify(dsl), function(err, raw){
			...
			res.aggregations.AGG.buckets.forEach(function(d){
				...
				d.AGG.buckets.forEach(function(k)	{
					...
					stats.hari = hari;
					stats.masa = time;
					M.data.hari_hourly.push(stats);
				});
			});


			dbg&&console.log('M.data.hari', M.data.hari);
			dbg&&console.log('M.data.hari_hourly', M.data.hari_hourly);
...
function viz3_render(cb)	{
	...
	var nest = d3.nest()
		.key(function(d){ return d.hari })
		.key(function(d){ return d.masa })
		.entries( M.data.hari_hourly )


	dbg&&console.log('nest', nest);


	var sel = d3.select('#container4');
...
//		scales[d.key] = d3.scaleSqrt().domain([d.min,d.max]).range([colors[0],'lime']);
	...
	nest.sort(d3.comparator().order(d3.ascending,
		function(d){ return ['MONDAY','TUESDAY','WEDNESDAY','THURSDAY','FRIDAY','SATURDAY','SUNDAY'].indexOf(d.key) })
	);
	...
	sel.append('h3')
		...
		.html('Collective Available Parking Spots by DAY of Week');
